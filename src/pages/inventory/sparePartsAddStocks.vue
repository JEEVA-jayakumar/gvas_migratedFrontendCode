    ...
    <div>
      ...
      <div class="col-md-12 text-h6 q-px-lg q-py-md text-weight-regular bottom-border text-grey-9">
        ...
        <q-table :rows="tableData" table-class="customSATableClass" :columns="columns" v-model:pagination="paginationControl" row-key="name" color="grey-9">
          <template v-slot:body-cell-created_date="props"><q-td :props="props">{{ $moment(props.row.created_date).format("Do MMM Y") }}</q-td></template>
          <template v-slot:body-cell-updated_date="props"><q-td :props="props">{{ $moment(props.row.updated_date).format("Do MMM Y") }}</q-td></template>
        </q-table>
      </div>
    </div>
  </q-page>
