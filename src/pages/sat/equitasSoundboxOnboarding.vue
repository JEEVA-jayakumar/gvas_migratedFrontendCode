<template>
  <q-page>
    ...
    >
      <q-tab color="dark" name="unassigned" label="Un Assigned" />
        ...
        >
          <template v-slot:body-cell-leadNumber="props">
            <q-td :props="props" class="cursor-pointer">
              <span class="label text-primary">{{ props.row.qrLeadInformation.id }}</span>
            </q-td>
          </template>
...
export default {
  ...
  data() {
    return {
      ...
      columnDataUnassigned: [
        ...
        { name: "tid", required: true, label: "TID", align: "left", field: "tid", sortable: false },
        { name: "mid", required: true, label: "MID", align: "left", field: "mid", sortable: false },
  ...
  methods: {
    ...
    ajaxLoadAllLeadInfo1({ pagination, filter }) {
      ...
      this.EQUITAS_ONBOARDING_QUEUE_UNASSIGNED_LIST({ pagination, filter }).then(() => {
        ...
        .catch(error => {
          this.$q.loading.hide();
          this.tableData1 = [];
          this.$q.notify({ color: "negative", position: "bottom", message: error?.message || "No Data Found", icon: "thumb_down" });
        });
    },
    ajaxLoadAllLeadInfo({ pagination, filter }) {
      this.$q.loading.show({ delay: 0, spinnerColor: "purple-9", message: "Fetching data .." });
      this.EQUITAS_ONBOARDING_QUEUE_ASSIGNED_LIST({ pagination, filter }).then(() => {
    ...
    ajaxLoadAllLeadInfo2({ pagination, filter }) {
      this.$q.loading.show({ delay: 0, spinnerColor: "purple-9", message: "Fetching data .." });
      this.EQUITAS_ONBOARDING_QUEUE_COMPLETED_LIST({ pagination, filter }).then(() => {
    ...
    assignImplementationUser() {
      ...
      if (self.formData.marsDeviceIdsCooked.length == 0) {
        ...
        let ids = self.formData.marsDeviceIdsCooked.map(v => v.id);
        let postValues = { userId: this.formData.assignTo.id, marsDeviceIds: { marsDeviceIds: ids } };
        self.EQUITAS_SOUNDBOX_REQUEST_UNASSIGED_TO_ASSIGNED_STATE(postValues).then(res => {
            if (res.status == 200) {
      ...
      } else if (self.formData.region && !self.formData.assignTo.id) {
        this.$q.loading.show({ delay: 100, spinnerColor: "purple-9", message: "Please wait.." });
        let ids = self.formData.marsDeviceIdsCooked.map(v => v.id);
        let data = { regionCode: self.formData.region, marsDeviceIds: { marsDeviceIds: ids } };
        self.EQUITAS_ONBOARDING_REQUEST_TO_ASSIGNED_REGION(data).then(res => {
            if (res.status == 200) {
              this.formData.marsDeviceIdsCooked = [];
        ...
        self.$q.notify({ color: "negative", position: "bottom", message: "Select exactly one assignment mode", icon: "thumb_down" });
      }
    },
