...
export default {
  ...
  computed: {
    ...
    ...mapGetters("InventoryCentral", ["getAllInventoryDevicesTypesData", "getAllRegionsData", "getRegionBasedSO"])
  },
  ...
  methods: {
    ...
    ...mapActions("InventoryCentral", ["FETCH_ALL_INVENTORY_DEVICES_TYPES_DATA", "FETCH_ALL_REGIONS_DATA", "FETCH_REGION_BASED_SO"]),
    ...
    getDevice(val) {
      this.FETCH_APP_DEVICES_DATA(val.id)
        .then(() => {
          this.dropDown.deviceOptions = this.getAllAppDevicesInfo.map(item => ({
            value: item,
    ...
    fnSubmitBankDetails(formData) {
      ...
      if (this.v$.formData.$error) {
        ...
        const regionObj = typeof formData.region === 'string' ? JSON.parse(formData.region) : formData.region;
        const submitData = { ...formData, region: regionObj, vasInstanceMapping: this.formData.vasInstanceMapping };

        this.STATE_SHORT_LEAD(submitData).then(() => {
    ...
    devSelected() {
      ...
      this.FETCH_VAS_DATAS(this.formData.device.id).then((response) => {
        if (response.status == 200) {
          this.$q.loading.hide()
          this.selectBankEnableOptions = this.getVasDeviceMapping.map(value => ({
    ...
    fnPincodeSearch(val, update) {
      ...
      this.FETCH_PINCODE_WITH_TERM(val).then(() => {
        update(() => {
          this.pincodeOptionsSearch = this.getAllStatesData.filter(oo =>
            oo.label.toString().includes(val.toLowerCase())
          );
        });
      }).catch(() => {
        update(() => { this.pincodeOptionsSearch = []; });
    ...
    appLoadData() {
      this.FETCH_ALL_REGIONS_DATA().then(() => {
        this.dropDown.regionOptions = this.getAllRegionsData.map(item => ({
      ...
      this.FETCH_APP_LEADSOURCE_DATA().then(() => {
        ...
        const isVaraneek = userInfo && userInfo.region && userInfo.region.regionAreaName == 'VARANEEK';

        this.dropDown.leadSourceOptions = this.getAllAppLeadSource
          .filter(item => item.active && (!isVaraneek || item.sourceName == 'VARANEEK'))
