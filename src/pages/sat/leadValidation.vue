          ...
          <template v-slot:body-cell-action="props">
            <q-td :props="props">
              ...
              <q-btn v-else-if="props.row.leadStatus == $LEAD_STATUS_SUBMITED_TO_MARS" push disable
                color="purple-9" size="sm" label="Pending with MARS" />
              ...
              <q-btn v-else-if="props.row.leadStatus == $LEAD_STATUS_REFER_BACK" push outline color="amber-9"
                size="sm" @click="$router.push('/sat/lead/validation/'+ props.row.leadId)" label="Referred Back" />

              <q-btn v-else-if="props.row.leadStatus == $LEAD_STATUS_MARS_SUB_TID_PENDING" push outline
                color="amber-9" size="sm"
                @click="$router.push('/sat/lead/validation/'+ props.row.leadId+'/data/entry')" label="Sub Tid Validation" />
              ...
              <q-btn v-else-if="props.row.leadStatus == $LEAD_STATUS_MARS_REFERRAL_BACK_DATA_ENTRY_PENDING" push
                ...
                @click="$router.push('/sat/lead/validation/'+ props.row.leadId+'/data/entry')" label="RB - data entry" />

              <q-btn v-else disable push outline color="grey-9" size="sm" label="Validate" />
            </q-td>
          </template>
