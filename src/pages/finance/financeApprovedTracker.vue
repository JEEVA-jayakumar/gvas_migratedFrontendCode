...
export default {
  ...
  data() {
    return {
      ...
      filter1: "",
      columns: [
        { name: "leadNumber", required: true, label: "Lead Number", align: "left", field: (row) => "# " + row.leadNumber, sortable: true },
        { name: "leadName", required: true, label: "Merchant Name", align: "left", field: "merchantName", sortable: true },
        { name: "assignedTo.name", required: true, label: "SO Name", align: "left", field: "soName", sortable: true },
        { name: "createdAt", required: true, label: "Submission Date", align: "left", field: "submissionDate", sortable: true },
        { name: "submittedToFinanceDate", required: true, label: "Date", align: "center", field: "submittedToFinanceDate", sortable: true },
        { name: "referenceNumber", required: true, label: "Payment Reference", align: "left", field: "paymentReference", sortable: true },
        { name: "setupFees", required: true, label: "Amount (INR)", align: "left", field: "amount", sortable: true },
        { name: "updatedAt", required: true, label: "TID Activation Date", align: "left", field: "leadLastUpdated", sortable: true }
      ],
      columns1: [
        { name: "qrLeadNumber", required: true, label: "Lead Number", align: "left", field: row => "# " + row.qrLeadNumber, sortable: true },
        { name: "leadName", required: true, label: "Merchant Name", align: "left", field: "merchantName", sortable: false },
        { name: "assignedTo.name", required: true, label: "SO Name", align: "left", field: "soName", sortable: false },
        { name: "createdAt", required: true, label: "Submission Date", align: "left", field: "submissionDate", sortable: false },
        { name: "referenceNumber", required: true, label: "Payment Reference", align: "left", field: row => row.paymentReference == null ? 'NA' : row.paymentReference, sortable: false },
        { name: "setupFees", required: true, label: "Amount (INR)", align: "left", field: "amount", sortable: false }
      ],
      tableData: [],
      tableData1: []
    };
  ...
  methods: {
    ...
    ajaxLoadAllFinanceApprovedData({ pagination, filter }) {
      ...
      this.FETCH_ALL_APPROVED_FINANCE_DATA({ pagination, filter }).then(() => {
    ...
    ajaxLoadAllFinanceQrApprovedData({ pagination, filter }) {
      this.$q.loading.show({ delay: 0, spinnerColor: "purple-9", message: "Fetching data .." });
      this.FETCH_ALL_APPROVED_QR_FINANCE_DATA({ pagination, filter }).then(() => {
          this.paginationControl1 = pagination;
          this.paginationControl1.rowsNumber = this.getstaticQrApprovedTrackerData.totalElements;
          this.paginationControl1.page = this.getstaticQrApprovedTrackerData.number + 1;
    ...
    downloadReport() {
      ...
      this.REPORT_FINANCE_APPROVED(this.formData).then(() => {
          this.$q.loading.hide();
    ...
    downloadQrReport() {
      this.$q.loading.show({ delay: 100 });
      this.REPORT_QR_FINANCE_APPROVED(this.formData).then(() => {
          this.$q.loading.hide();
          this.$q.notify({ color: "positive", position: "bottom", message: "Success, file has been downloaded", icon: "check" });
        }).catch(() => {
          this.$q.loading.hide();
