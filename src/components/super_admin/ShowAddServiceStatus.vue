      ...
      <q-card style="min-width: 30vw">
        <form>
          <div class="row q-pa-md items-center border-bottom">
            <div class="col-md-12">
              <div class="text-h6 text-weight-regular">Add Service Status</div>
            </div>
          </div>
          <div class="row q-pa-md items-center">
            <div class="col-md-12 full-width">
              <q-input
                ...
                @keyup.enter="fnfinalsubmitAddServiceStatus()"
              />
            </div>
          </div>
          <div class="row q-pa-md items-center justify-end">
            <q-btn
              ...
              @click="emitfnShowAddNewServiceStatus()"
              label="Cancel"
            />
            <q-btn
              @click="fnfinalsubmitAddServiceStatus()"
              color="purple-9"
              label="Save"
            />
          </div>
...
export default {
  ...
  methods: {
    ...mapActions("ServiceRequestStatus", ["POST_SERVICE_STATUS_TYPES"]),
    emitfnShowAddNewServiceStatus() {
      this.$emit("emitfnShowAddNewServiceStatus");
    },
    fnfinalsubmitAddServiceStatus() {
      this.v$.formData.$touch();
      if (this.v$.formData.$error) {
        this.$q.notify("Please review fields again.");
      } else {
        this.$q.loading.show({ message: "Please Wait" });
        this.POST_SERVICE_STATUS_TYPES(this.formData)
          .then(() => {
            this.$q.loading.hide();
