...
export default {
  ...
  data() {
    return {
      ...
      formData: {
        serviceReqType: {
          name: this.propRowDetails2.serviceReqType.name,
        },
      },
    };
  },
  ...
  methods: {
    ...
    initializeSelections() {
        this.selectedIssues = this.propRowDetails2.serviceReqIssueTypeSets.map(s => s.serviceReqIssueType);
        this.selectedStatuses = this.propRowDetails2.serviceRequestStatusSets.map(s => s.serviceRequestStatus);
    },
