...
export default {
  ...
  methods: {
    ...
    ajaxLoadAllLeadInfo1({ pagination, filter }) {
      ...
      this.PHONEPE_IMPLEMENTATION_QUEUE_COURIER_LIST({ pagination, filter }).then((res) => {
    ...
    UpdateDeviceAddress(currentDeviceInfo) {
      ...
      let ids = this.formData.marsDeviceIdsCookedUnAssinged.length == 0 ? [currentDeviceInfo.id] : this.formData.marsDeviceIdsCookedUnAssinged.map(v => v.id);
      this.currentDeviceInfo = {
        id: ids,
        marsDeviceAddress: {
    ...
    reAssignImplementationUser() {
      if (this.formData.marsDeviceIdsCookedUnAssinged.length == 0 || this.formData.assignTo == "") {
        this.$q.notify({ color: "negative", position: "bottom", message: "Select items and user to assign", icon: "thumb_down" });
        return;
      }
      this.$q.loading.show({ delay: 100, spinnerColor: "purple-9", message: "Fetching Data.." });
      let postValues = {
