  ...
  <q-dialog persistent v-model="toggleModel">
    <q-card style="min-width: 400px">
      ...
      <q-card-actions align="end" class="q-pa-md">
        ...
        <q-btn label="Submit" color="purple-9" @click="fnEDITREOPEN" />
      </q-card-actions>
    </q-card>
  </q-dialog>
</template>
...
export default {
  ...
  methods: {
    ...
    fnEDITREOPEN() {
      this.$q.dialog({ title: "Confirm", message: "Are you sure want to reject the lead?", ok: "Continue", cancel: "Cancel" }).onOk(() => {
        this.$q.loading.show({ message: "Processing .." });
        const param = {
          userId: this.formData.so,
          request: [{
            ...this.propRowDetails,
            reAssignReason: this.formData.Reassign,
            reAssignRemark: this.formData.reason,
            subTicketsList: null
          }]
        };
        this.REASSIGNED_REASON_TYPE_DETAILS(param).then(() => {
